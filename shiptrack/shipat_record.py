...
Usage: shipat_record.py <ship> <lat> <lon> <speed>

Prints one tab-separated row: ship, lat, lon, speed, current time (UTC, when
...
def main():
    if len(sys.argv) != 5:
        raise SystemExit(f"Usage: {sys.argv[0]} <ship> <lat> <lon> <speed>")

    ship, lat_s, lon_s, speed_s = sys.argv[1:5]
    lat, lon, speed = float(lat_s), float(lon_s), float(speed_s)
